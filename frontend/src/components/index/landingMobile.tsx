import RegisterButton from "../buttons/RegisterButton";

export default function LandingMobile() {
  return (
    <header
      aria-labelledby="hero-heading"
      className="px-6 py-16 bg-[linear-gradient(90deg,rgba(2,0,36,1)_0%,rgba(104,171,124,1)_0%,rgba(32,34,65,1)_82%,rgba(29,37,41,1)_100%,rgba(0,212,255,1)_100%)]"
    >
      <h1 id="hero-heading" className="text-5xl font-bold mb-4 text-light">
        Welcome to The Pu-Erh Catalogue
      </h1>
      <p className="text-lg text-light max-w-2xl pb-5">
        Discover, rate, and catalog teas from around the world. Join the
        community and keep track of your favorite teas.
      </p>
      <RegisterButton
        text="Start steeping!"
        className="nohover bg-orange-500 hover:bg-orange-muted transition-all text-white font-semibold px-6 py-3 rounded-full shadow-lg"
      />
    </header>
  );
}
